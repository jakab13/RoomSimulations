...
% Labels:
% -------
%   materials       (true) If wall materials are specified in a cell array (see RAZR),
...
if isfield(rooms, 'door') && ~isstruct(rooms(1).door)
    rooms = convert_door(rooms, {});
    one_warning('Use new format for doors. Doors have been converted');
end
...
for r = 1:numRooms;
    ...
    %% materials:
    if isfield(room, 'materials') && iscell(room.materials) && p.Results.materials
        mpos = zeros(6,3);                                          % text positions
        mpos(wall2sub(-3), :) = [1 1 0].*b * 1/2;                   % -z
        mpos(wall2sub(-2), :) = [1 0 1].*b * 1/2;                   % -y
        mpos(wall2sub(-1), :) = [0 1 1].*b * 1/2;                   % -x
        mpos(wall2sub(+1), :) = mpos(wall2sub(-1), :) + [1 0 0].*b; % +x
        mpos(wall2sub(+2), :) = mpos(wall2sub(-2), :) + [0 1 0].*b; % +y
        mpos(wall2sub(+3), :) = mpos(wall2sub(-3), :) + [0 0 1].*b; % +z
        mpos = mpos + repmat(room.origin, 6, 1);
        
        isstr = cellfun(@ischar, room.materials);
        mat_str = room.materials;
        mat_str(~isstr) = cellfun(@(x) mat2str(x, 2), room.materials(~isstr), ...
            'UniformOutput', false);
        
        h.text = text(mpos(:, 1), mpos(:, 2), mpos(:, 3), mat_str, ...
