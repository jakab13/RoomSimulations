...
%               If length(ir) > 1, the first dimension represents the ir, then
%               squeeze(iacc_matrix(n, :, :)) is in the same format as above for the n-th ir.
...
if length(ir) > 1
    for n = length(ir):-1:1
        [iacc_matrix(n, :, :), freq, ranges] = iacc(ir(n), varargin{:});
    end
    return;
end
