...
%   title       ('') Title for axes (EDC and RT). If not specified and length(ir) > 1, axes will be
%               labeled with ascending numbers.
%
% Output:
...
if length(ir) > 1
    if length(ir) > 10
        answ = input('schroeder_rt: Create more than 10 figures (Y/n)? ', 's');
        if ~strcmpi(answ, 'y') && ~isempty(answ)
            return;
        end
    end
    
    title_idx = find(strcmp('title', varargin), 1, 'last');
    if ~isempty(title_idx)
        titles = repmat(varargin(title_idx + 1), 1, length(ir));
    else
        titles = strsplit(num2str(1:length(ir)));
    end
    
    narg = length(varargin);
    
    for n = 1:length(ir)
        varargin(narg + (1:2)) = {'title', titles{n}};
        if nargout == 0
            schroeder_rt(ir(n), varargin{:});
        else
            [rt(n, :), freq_out, edc_mean_lr(n, :, :), tvec{n}, handles(n)] = ...
                schroeder_rt(ir(n), varargin{:});
        end
    end
...
if p.Results.plot_rt
    ...
    xlabel('Frequency (Hz)');
    if ischar(p.Results.measure)
        title_str = upper(p.Results.measure);
    else
        title_str = sprintf('EDC fit interval: [%g, %g] dB', yfitrange(1), yfitrange(2));
    end
    if ~isempty(p.Results.title)
        title_str = [title_str, ' (', p.Results.title, ')'];
    end
    title(title_str);
