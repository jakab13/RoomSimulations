function [] = sPwrVis(inSig, lsPos, VRSangles, dRange)
...
% - Implement VRS Position Plotting
%%%%%%%%%%
...
%     [sphX,sphY,sphZ] = sph2cart(az,el,distSph);
    ...
    for k = 1:length(sphX)
        if pwrVecSize(k)>b
            plot3(sphX(k),sphY(k),sphZ(k),'o','MarkerSize',pwrVecSize(k),'Color','r','MarkerFaceColor',cMap(pwrVecColor(k),:))
        else
            plot3(sphX(k),sphY(k),sphZ(k),'o','MarkerSize',pwrVecSize(k),'Color','k','MarkerFaceColor',[1 1 1])
        end
    end
    if ~isempty(VRSangles)
