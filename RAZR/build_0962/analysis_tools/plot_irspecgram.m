...
if length(ir) > 1
    if length(ir) > 10
        answ = input('plot_irspecgram: Create more than 10 figures (Y/n)? ', 's');
    ...
    for n = length(ir):-1:1
        [hfig(n), hax(n, :), hplot(n, :), pow(n, :), freq, t_sec] = plot_irspecgram(ir(n), winlen_sec, t_max);
        try
            for k = 1:length(hax(n, :))
                hax(n, k).Title.String = sprintf('%s (%d)', hax(n, k).Title.String, n);
            end
        catch
        end
    end
