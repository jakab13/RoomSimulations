...
%   RAZR rmpath     Removes the added folders from search path
...
%                           az: [positive, negative] <-> [left, right],
%                           el: [-90, 90] <-> [down, up]
%
...
%                           - A cell array, one entry per wall, with each entry
%                             being either a material string, a row vector, or
%                             a scalar number.
...
%           tain an extra field: a door, being a structure itself. The following
%           fields are required:
%
...
%           The rooms vector must in total contain one source and one receiver
...
%           or not specified, default options are applied. All available options
%           are listed in GET_DEFAULT_OPTIONS. See also the search tool SOP.
...
%
%           This specifies a connection from room 'roomA' via door 'door_A1' to
%           room 'roomB' via door 'door_B1' and a connection from room 'roomA'
%           via door 'door_A2' to room 'roomC' via door 'door_C1'. An example to
%           simulate two coupled rooms is provided in EXAMPLE_COUPLED.M.
%
...
%               - name          Name of impulse response (specified as
%                               op.rirname; otherwise: same as room.name)
%
%               Additional fields may be returned, depnding on passed options.
%               Type "sop return" to display respective options.
%
...
elseif isstruct(varargin{1})
    ...
    switch varargin{1}
        ...
        case 'about'
            ...
            % assume that the web link is the last line to be printed:
            pattern = 'razrengine.com';
