...
if op.ism_enableReflFilt
    [ism_setup.b_refl, ism_setup.a_refl] = getReflFiltCoeff(room.freq, room.reflcoeff, ...
        ism_setup.b_bp, ism_setup.a_bp, op);
else
...
if op.enableSR(1)
    ...
    % normalvec_dir):
    strDelays = strDelays(wall2sub(normalvec_dir));
...
elseif op.legacy_rand
    if length(orders) == 2
        ism_setup.orders_from_to = repmat(orders', 1, 2);
    else
        ism_setup.orders_from_to = [orders([1, (end - 1)]); orders([end, end])];
    end
...
if op.legacy_rand
    ism_setup.rng_seeds = repmat(seed_tmp, size(ism_setup.orders_from_to, 1), 1);
else
