function hrtf_dbase = get_hrtf_dbase(database, options, force_loading)
...
persistent hrtf_string
persistent hrtf_options
persistent dbase
%         strcmp(hrtf_string, database) && ...

if ~isempty(dbase)
% this is to avoid having to frequently reload the HRTF database.
% However, it tends to be buggy AF
if ...
        strcmp(dbase.input_string, database) && ...
        isequal(hrtf_options, options) && ...
        ~isempty(dbase) && ...
        ~force_loading
    hrtf_dbase = dbase;
    return;
...
hrtf_string = database;
hrtf_options = options;
