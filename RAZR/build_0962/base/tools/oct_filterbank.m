...
for n = 1:num_f0
    ...
    % SE 20.04.2021 sanity checks and tweaks
    if f1(n) < 0.002
        % here we might get unstable, hacky use lower order
        % instead we should use zp design and sos
        % Zero-Pole-Gain design
        %  [z,p,k]= butter(nButter, [f1(n), f2(n)]);
        %  sos = zp2sos(z,p,k);
        warning('lowest octave filter(s) use(s) order 3 instead of 4 for stability');
        [b_tmp, a_tmp] = butter(nButter-1, [f1(n), f2(n)]);
        b(n+includeEdges, :) = [b_tmp, zeros(1, 2)];
        a(n+includeEdges, :) = [a_tmp, zeros(1, 2)];
    elseif f1(n) > 1
        error('octave filter out of range');
    elseif f2(n) > 1
        warning('octave filter reaches fs/2');
        [b_tmp, a_tmp] = butter(nButter, f1(n), 'high');
        b(n+includeEdges, :) = [b_tmp, zeros(1, nButter)];
        a(n+includeEdges, :) = [a_tmp, zeros(1, nButter)];
    else
