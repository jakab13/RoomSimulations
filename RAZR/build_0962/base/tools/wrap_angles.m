function [azel, el] = wrap_angles(azel, unit)
% WRAP_ANGLES - Wrap azimuth and elevation angles to the ranges [-180, 180] and
% [-90, 90], respectively.
%
% Usage:
%   [azimelev] = WRAP_ANGLES(azel, unit)
% or:
%   [azim, elev] = WRAP_ANGLES(azel, unit)
%
% Input:
%   azel        Matrix containing corresponding azimuth and elevation angles in columns
%   unit        Angle unit, 'deg' or 'rad'
%
% Output:
...
% compatibility of older version, where second input parameter was boolean flag
% 'isdeg':
if ~ischar(unit)
    if unit  % i.e. isdegree == true
        unit = 'deg';
    else
        unit = 'rad';
    end
